...
//    }
    ...
    private fun calculateTotalAmount(): Int {
        var totalAmount = 0
        for (i in foodItemsPrice.indices) { // Use indices for better readability
            try {
                // Remove the currency symbol and trim whitespace
                val price = foodItemsPrice[i].replace("₹", "").trim()
                // Convert to integer
                ...
                // Log the error
                Log.e("PayOutActivity", "Error parsing price: ${foodItemsPrice[i]}", e)
                // Optionally, you can show a message to the user
                Toast.makeText(this, "Invalid price format for ${foodItemsPrice[i]}", Toast.LENGTH_SHORT).show()
            }
        }
        return totalAmount
    }
